...
GO
PRINT N'Dropping [dbo].[VwUser].[MS_DiagramPane1]...';


GO
EXECUTE sp_dropextendedproperty @name = N'MS_DiagramPane1', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'VwUser';


GO
PRINT N'Dropping [dbo].[VwUser].[MS_DiagramPaneCount]...';


GO
EXECUTE sp_dropextendedproperty @name = N'MS_DiagramPaneCount', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'VwUser';


GO
PRINT N'Dropping [dbo].[DF__User__CreatedDat__3AC1AA49]...';


GO
ALTER TABLE [dbo].[User] DROP CONSTRAINT [DF__User__CreatedDat__3AC1AA49];


GO
PRINT N'Dropping [Accounting].[spChangePrice]...';


GO
DROP PROCEDURE [Accounting].[spChangePrice];


GO
PRINT N'Dropping [Accounting].[UddtChangePrice]...';


GO
DROP TYPE [Accounting].[UddtChangePrice];
...
GO
PRINT N'Starting rebuilding table [Accounting].[LastPrice]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_LastPrice] (
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[LastPrice])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_LastPrice] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_LastPrice] ([Id], [StockId], [GoodId], [CreateId], [CreateDate], [PriceBuy], [PriceRetail], [PriceWholeSale], [LastPriceRetail], [LastPriceWholeSale], [Description])
        SELECT [Id],
               [StockId],
               [GoodId],
               [CreateId],
               ...
               [Description]
        FROM   [Accounting].[LastPrice];
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_LastPrice] OFF;
    END

DROP TABLE [Accounting].[LastPrice];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_LastPrice]', N'LastPrice';
...
WHERE Id=(SELECT MAX(s.Id) FROM Accounting.Stock s WHERE s.GoodId=@goodId)
	...
	INSERT INTO Accounting.LastPrice (StockId,GoodId,PriceBuy,PriceRetail,PriceWholeSale,LastPriceBuy,LastPriceRetail,LastPriceWholeSale,Description,CreateId,CreateDate) 
