...
GO
/*
The column [Accounting].[Cash].[CashId] is being dropped, data loss could occur.
*/

IF EXISTS (select top 1 1 from [Accounting].[Cash])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT

GO
PRINT N'Dropping [Accounting].[DF__tmp_ms_xx__Creat__7D39791C]...';


GO
ALTER TABLE [Accounting].[Cash] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__7D39791C];


GO
PRINT N'Starting rebuilding table [Accounting].[Cash]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_Cash] (
    ...
    [IsActive]    BIT             NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PKCash1] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[Cash])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Cash] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_Cash] ([Id], [CompanyId], [CashName], [Balance], [Description], [CreateId], [CreateDate], [IsActive])
        SELECT   [Id],
                 [CompanyId],
                 [CashName],
                 [Balance],
        ...
        FROM     [Accounting].[Cash]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Cash] OFF;
    END

DROP TABLE [Accounting].[Cash];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_Cash]', N'Cash';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_PKCash1]', N'PKCash', N'OBJECT';
...
GO
PRINT N'Altering [Accounting].[VwDeficitGoodStock]...';


GO
ALTER VIEW [Accounting].[VwDeficitGoodStock]
	AS  
...
GO
PRINT N'Altering [Accounting].[VwIncludeReport]...';


GO
ALTER VIEW [Accounting].[VwIncludeReport]
	AS select
DD.GoodName
,dd.Quantity
,dd.PriceBuy  
...
GO
PRINT N'Altering [Accounting].[spPaymentDebt]...';


GO
ALTER PROCEDURE [Accounting].[spPaymentDebt]
	@userId int,
...
GO
PRINT N'Refreshing [Accounting].[spCashFromMoney]...';


GO
EXECUTE sp_refreshsqlmodule N'[Accounting].[spCashFromMoney]';


GO
PRINT N'Refreshing [Accounting].[spCashToMoney]...';


GO
EXECUTE sp_refreshsqlmodule N'[Accounting].[spCashToMoney]';
...
GO
PRINT N'Refreshing [dbo].[spCashPermitted]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[spCashPermitted]';
