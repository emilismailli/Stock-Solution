...
GO
PRINT N'Dropping unnamed constraint on [Accounting].[Payment]...';


GO
ALTER TABLE [Accounting].[Payment] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__71C7C670];


GO
PRINT N'Starting rebuilding table [Accounting].[Payment]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_Payment] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PKPayment1] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[Payment])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Payment] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_Payment] ([Id], [CashId], [CashFromId], [ExpenseId], [AccountId], [CustomerId], [CustomerWareHouseId], [DocumentId], [PaymentTypeId], [Payment], [Deficit], [Description], [Percent], [CreateDate])
        SELECT   [Id],
                 [CashId],
                 [CashFromId],
                 [ExpenseId],
        ...
        FROM     [Accounting].[Payment]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Payment] OFF;
    END

DROP TABLE [Accounting].[Payment];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_Payment]', N'Payment';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_PKPayment1]', N'PKPayment', N'OBJECT';
