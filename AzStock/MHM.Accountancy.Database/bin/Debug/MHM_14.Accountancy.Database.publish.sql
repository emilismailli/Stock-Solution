...
GO
PRINT N'Creating [Accounting].[UddtOfflineGood]...';


GO
CREATE TYPE [Accounting].[UddtOfflineGood] AS TABLE (
    [Id]                 INT             NOT NULL,
    [StoGoodId]          INT             NULL,
...
GO
PRINT N'Creating [Accounting].[Offline]...';


GO
CREATE TABLE [Accounting].[Offline] (
    ...
    [IsFiltered]      BIT            NULL,
    CONSTRAINT [PKOffline] PRIMARY KEY CLUSTERED ([Id] ASC)
);


GO
PRINT N'Creating [Accounting].[OfflineDetail]...';


GO
CREATE TABLE [Accounting].[OfflineDetail] (
    [Id]              INT             IDENTITY (1, 1) NOT NULL,
    [BarcodeId]       INT             NULL,
    [DocumentId]      INT             NULL,
    [AccountId]       INT             NULL,
    ...
    [DeleteDate]      DATETIME        NULL,
    [ReadOnly]        DATETIME        NULL,
    CONSTRAINT [PKOfflineDetail] PRIMARY KEY CLUSTERED ([Id] ASC)
);


GO
PRINT N'Creating unnamed constraint on [Accounting].[Offline]...';


GO
ALTER TABLE [Accounting].[Offline]
...
GO
PRINT N'Creating unnamed constraint on [Accounting].[OfflineDetail]...';


GO
ALTER TABLE [Accounting].[OfflineDetail]
...
GO
PRINT N'Altering [Accounting].[VwInventar]...';


GO
ALTER VIEW [Accounting].[VwInventar]
	AS 
	SELECT 
	i.FullGoodName,
...
GO
PRINT N'Creating [Accounting].[VwOffline]...';


GO
CREATE VIEW [Accounting].[VwOffline]
	AS SELECT 
	...
	FROM [Accounting].[Offline] o
		left join [Accounting].[VwCompany] c on o.CompanyId=c.Id
	left join [WareHouses].[VwWareHouse] wh on o.WareHouseId=wh.Id
	left join [WareHouses].[VwWareHouse] whf on o.FromWarehouseId=whf.Id
	left join Accounting.VwContract co on o.ContractId=co.Id 
	
	left join Accounting.VwPaymentType pt on o.PaymentTypeId=pt.Id
GO
PRINT N'Creating [Accounting].[VwOfflineDetail]...';


GO
CREATE VIEW [Accounting].[VwOfflineDetail]
	AS SELECT
...
,od.Deficit
	...
	FROM [Accounting].[OfflineDetail] od


	left join [Accounting].[VwGood] g on od.GoodId=g.Id 
	left join [dbo].[Unit] u on od.UnitId=u.Id
GO
...
GO
PRINT N'Altering [Accounting].[spChangePrice]...';


GO
ALTER PROC Accounting.spChangePrice
@userId int,
...
GO
PRINT N'Creating [Accounting].[spOfflineDocument]...';


GO
CREATE PROCEDURE [Accounting].[spOfflineDocument]
	@userId int ,
	@operationType tinyint = null,
	@id INT = NULL,
	@menuId INT,
	...
	@detail [Accounting].UddtOfflineGood readonly
...
INSERT INTO [Accounting].[Offline]( [IncludeDate],[PaymentDate],[CompanyId],[WareHouseId],[Description],[PaymentTypeId],[ContractId],[SaleNo],Accounting.Document.CreateId,AccountId)
VALUES (@includeDate,@paymentDate,@companyId,@wareHouseId,@description,@paymentTypeId,@contractId,[Accounting].[SFnNewSaleNo](@userId,@accountCode),@userId,7)

SET @id=SCOPE_IDENTITY();



   INSERT INTO [Accounting].[OfflineDetail]
   ([DocumentId]
   ,[AccountId]
   ,[GoodId]
   ,[BarCodeId]
...
,priceWholeSale
,Quantity*priceBuy
,Quantity*priceRetail
,Quantity*priceWholeSale
,payment
,[percent]
,userId
,Getdate()
...
select o.[Id]  
	...
	,o.[CustomerName]
         ...
         ,max(od.AmountBuy) AmountBuy
         ,max(od.AmountRetail) AmountRetail
         ,max(od.AmountWholeSale) AmountWholeSale
		 ,max(od.PriceBuy) PriceBuy
		 ,max(od.PriceRetail) PriceRetail
		 ,max(od.PriceWholeSale) PriceWholeSale
	     ,max(od.Payment) Payment
	     ,max(od.Deficit) Deficit
from [Accounting].[VwOffline] o
 left join [Accounting].[OfflineDetail] od on o.Id=od.DocumentId
 left join [Accounting].[VwContract] c on c.id=o.ContractId
 JOIN dbo.WarehouseMenuCollection wmc ON wmc.WareHouseId = o.WareHouseId and wmc.UserId=@userId AND wmc.Allow=1 AND wmc.MenuId=@menuId
 where AccountId=7 and o.DeleteId is null
 group by 
     o.[Id]  
    ,o.[IncludeDate]   
    ,o.[CompanyId]   
