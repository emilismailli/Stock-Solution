...
GO
PRINT N'Dropping unnamed constraint on [Accounting].[Document]...';


GO
ALTER TABLE [Accounting].[Document] DROP CONSTRAINT [DF__tmp_ms_xx__Inclu__52B92F6B];
...
GO
ALTER TABLE [Accounting].[Document] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__53AD53A4];


GO
PRINT N'Starting rebuilding table [Accounting].[Document]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_Document] (
    [Id]              INT             IDENTITY (1, 1) NOT NULL,
    [AccountId]       INT             NULL,
    [IncludeDate]     DATETIME        DEFAULT getdate() NULL,
    [CompanyId]       INT             NULL,
    [SaleNo]          INT             NULL,
    [ContractId]      INT             NULL,
    ...
    [IsFiltered]      BIT             NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PKDocument1] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[Document])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Document] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_Document] ([Id], [AccountId], [IncludeDate], [CompanyId], [SaleNo], [ContractId], [WareHouseId], [FromWarehouseId], [PaymentTypeId], [Description], [ReasonId], [PaymentDate], [PartnerOrderNo], [AdditionalCost], [MainNumber], [CreateId], [CreateDate], [UpdateId], [UpdateDate], [DeleteId], [DeleteDate], [ReadOnly], [DocTypeId], [DocCertId], [DocSeriesId], [IsFiltered])
        SELECT   [Id],
                 [AccountId],
                 [IncludeDate],
                 [CompanyId],
        ...
        FROM     [Accounting].[Document]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Document] OFF;
    END

DROP TABLE [Accounting].[Document];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_Document]', N'Document';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_PKDocument1]', N'PKDocument', N'OBJECT';
...
GO
PRINT N'Altering [Accounting].[VwDocumentReturnSale]...';


GO
ALTER VIEW [Accounting].[VwDocumentReturnSale]
	AS SELECT 
	 d.[Id]   
	,d.[AccountId]
	,ac.Code AccountCode
