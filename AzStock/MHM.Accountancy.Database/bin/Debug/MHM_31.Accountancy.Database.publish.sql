...
GO
PRINT N'Dropping unnamed constraint on [Accounting].[Logistic]...';


GO
ALTER TABLE [Accounting].[Logistic] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__288DEB75];


GO
PRINT N'Starting rebuilding table [Accounting].[Logistic]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_Logistic] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PKLogistic1] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[Logistic])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Logistic] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_Logistic] ([Id], [Driver], [Fin], [CompanyId], [WareHouseId], [Transportation], [Cart], [Sender], [SenderAddress], [SenderManager], [Received], [ReceivedAddress], [ReceivedManager], [Date], [DeliveryDate], [Description], [CreateId], [CreateDate], [UpdateId], [UpdateDate], [DeleteId], [DeleteDate], [ReadOnly], [DocTypeId], [DocCertId], [DocSeriesId])
        SELECT   [Id],
                 [Driver],
                 [Fin],
                 [CompanyId],
        ...
        FROM     [Accounting].[Logistic]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Logistic] OFF;
    END

DROP TABLE [Accounting].[Logistic];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_Logistic]', N'Logistic';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_PKLogistic1]', N'PKLogistic', N'OBJECT';
...
GO
PRINT N'Altering [Accounting].[VwLogistic]...';


GO
ALTER VIEW [Accounting].[VwLogistic]
	AS SELECT 
