...
GO
PRINT N'Dropping [Accounting].[spOfflineDocument]...';


GO
DROP PROCEDURE [Accounting].[spOfflineDocument];


GO
PRINT N'Dropping [Accounting].[UddtOfflineDetail]...';


GO
DROP TYPE [Accounting].[UddtOfflineDetail];


GO
PRINT N'Creating [Accounting].[UddtOfflineDetail]...';


GO
CREATE TYPE [Accounting].[UddtOfflineDetail] AS TABLE (
...
begin
begin transaction
insert into [Accounting].[Offline]( [Date],[DeliveryDate],[Description],[CreateId],[DocTypeId],[DocCertId],[DocSeriesId],[CompanyId],[WareHouseId],[PaymentTypeId],[ContractId])

VALUES                              (@date,@deliveryDate,@description,@userId,@docTypeId,@docCertId,@docSeriesId,@companyId,@wareHouseId,@paymentTypeId,@contractId);
 ...
 insert into [Accounting].[OfflineDetail]([LogisticId],[BarcodeId],[GoodId],[Quantity],[UnitId],[PriceBuy],[PriceWholeSale],[AmountBuy],[AmountWholeSale],[Description])
                                    select @id,[BarcodeId],[GoodId],[Quantity],[UnitId],[PriceBuy],[PriceWholeSale],[AmountBuy],[AmountWholeSale],[Description] from @details
	if @@TRANCOUNT>0
	COMMIT;
	if @operationType=11 goto spSelect;
RETURN 0
