...
GO
/*
The column [Accounting].[Offline].[Cart] is being dropped, data loss could occur.

The column [Accounting].[Offline].[DocCertId] is being dropped, data loss could occur.

The column [Accounting].[Offline].[DocSeriesId] is being dropped, data loss could occur.

The column [Accounting].[Offline].[DocTypeId] is being dropped, data loss could occur.

The column [Accounting].[Offline].[Driver] is being dropped, data loss could occur.

The column [Accounting].[Offline].[Fin] is being dropped, data loss could occur.

The column [Accounting].[Offline].[ReadOnly] is being dropped, data loss could occur.

The column [Accounting].[Offline].[Received] is being dropped, data loss could occur.

The column [Accounting].[Offline].[ReceivedAddress] is being dropped, data loss could occur.

The column [Accounting].[Offline].[ReceivedManager] is being dropped, data loss could occur.

The column [Accounting].[Offline].[Sender] is being dropped, data loss could occur.

The column [Accounting].[Offline].[SenderAddress] is being dropped, data loss could occur.

The column [Accounting].[Offline].[SenderManager] is being dropped, data loss could occur.

The column [Accounting].[Offline].[Transportation] is being dropped, data loss could occur.

The column [Accounting].[Offline].[UpdateDate] is being dropped, data loss could occur.

The column [Accounting].[Offline].[UpdateId] is being dropped, data loss could occur.
*/
...
GO
/*
The column [Accounting].[OfflineDetail].[IsActive] is being dropped, data loss could occur.

The column [Accounting].[OfflineDetail].[ReadOnly] is being dropped, data loss could occur.

The column [Accounting].[OfflineDetail].[SaleNo] is being dropped, data loss could occur.

The column [Accounting].[OfflineDetail].[UpdateDate] is being dropped, data loss could occur.

The column [Accounting].[OfflineDetail].[UpdateId] is being dropped, data loss could occur.
*/
...
GO
ALTER TABLE [Accounting].[Offline] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__2E46C4CB];
...
GO
ALTER TABLE [Accounting].[OfflineDetail] DROP CONSTRAINT [DF__tmp_ms_xx__Creat__38C4533E];
...
CREATE TABLE [Accounting].[tmp_ms_xx_Offline] (
    ...
    [DeleteId]      INT            NULL,
    [DeleteDate]    DATETIME       DEFAULT getdate() NULL,
    [CreateId]      INT            NULL,
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        ...
        INSERT INTO [Accounting].[tmp_ms_xx_Offline] ([Id], [CompanyId], [WareHouseId], [ContractId], [PaymentTypeId], [Date], [DeliveryDate], [Description], [CreateId], [CreateDate], [DeleteId], [DeleteDate])
        SELECT   [Id],
                 [CompanyId],
                 [WareHouseId],
                 [ContractId],
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        ...
        INSERT INTO [Accounting].[tmp_ms_xx_OfflineDetail] ([Id], [LogisticId], [BarcodeId], [GoodId], [Quantity], [PriceBuy], [PriceWholeSale], [AmountBuy], [AmountWholeSale], [UnitId], [Description], [CreateId], [CreateDate], [DeleteId], [DeleteDate])
        SELECT   [Id],
                 [LogisticId],
                 [BarcodeId],
                 [GoodId],
...
GO
PRINT N'Refreshing [Accounting].[IFnOfflineDetail]...';


GO
EXECUTE sp_refreshsqlmodule N'[Accounting].[IFnOfflineDetail]';
...
@description nvarchar(500)=null, 
	...
	@contractId int =null,
	@customerId int=null,
@details [Accounting].[UddtOfflineDetail]  readonly 
...
begin
begin transaction
insert into [Accounting].[Offline]( [Date],[DeliveryDate],[Description],[CreateId],[CompanyId],[WareHouseId],[PaymentTypeId],[ContractId])

VALUES                              (@date,@deliveryDate,@description,@userId,@companyId,@wareHouseId,@paymentTypeId,@contractId);

 set @id=SCOPE_IDENTITY();

 insert into [Accounting].[OfflineDetail]([LogisticId],[BarcodeId],[GoodId],[Quantity],[UnitId],[PriceBuy],[PriceWholeSale],[AmountBuy],[AmountWholeSale],[Description])
                                    select      @id   ,[BarcodeId],[GoodId],[Quantity],[UnitId],[PriceBuy],[PriceWholeSale],[AmountBuy],[AmountWholeSale],[Description]
...
,l.PaymentTypeName
         ...
         ,max(ld.AmountBuy) AmountBuy 
         ,max(ld.AmountWholeSale) AmountWholeSale
		 ,max(ld.PriceBuy) PriceBuy 
		 ,max(ld.PriceWholeSale) PriceWholeSale
