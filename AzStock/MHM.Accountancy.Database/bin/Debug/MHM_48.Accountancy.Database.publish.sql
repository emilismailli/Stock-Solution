...
GO
ALTER TABLE [Accounting].[LogisticDetail] DROP CONSTRAINT [DF__LogisticD__IsAct__06A2E7C5];
...
GO
ALTER TABLE [Accounting].[LogisticDetail] DROP CONSTRAINT [DF__LogisticD__Creat__07970BFE];
...
VALUES                              (@date,@deliveryDate,@description,@userId,@companyId,@wareHouseId,@paymentTypeId,@contractId);
 ...
 insert into [Accounting].[OfflineDetail]([LogisticId],[BarcodeId],[GoodId],[Quantity],[UnitId], [PriceWholeSale], [AmountWholeSale],[Description])
                                    select      @id   ,[BarcodeId],[GoodId],[Quantity],[UnitId], [PriceWholeSale], [AmountWholeSale],[Description]
...
,@description,@userId,@docTypeId,@docCertId,@docSeriesId,@companyId,@wareHouseId,@paymentTypeId,@contractId);
 ...
 insert into [Accounting].[OfflineSaleDetail]([LogisticId],[BarcodeId],[GoodId],[Quantity],[UnitId],[Description])
                                    select @id,[BarcodeId],[GoodId],[Quantity],[UnitId],[Description] from @details
	if @@TRANCOUNT>0
	COMMIT;
	if @operationType=11 goto spSelect;
