...
GO
PRINT N'Dropping [Accounting].[spDocumentSaleReturn]...';


GO
DROP PROCEDURE [Accounting].[spDocumentSaleReturn];


GO
PRINT N'Dropping [Accounting].[UddtSaleGoodReturn]...';


GO
DROP TYPE [Accounting].[UddtSaleGoodReturn];
...
INSERT INTO [Accounting].[Document]( [IncludeDate],[CompanyId],[WareHouseId],[Description],[PaymentTypeId],[ContractId], [CreateId],AccountId,[PaymentDate],[PartnerOrderNo])
VALUES (@includeDate,@companyId,@wareHouseId,@description,@paymentTypeId,@contractId, @userId,5,@paymentDate,@partnerOrderNo)

SET @id=SCOPE_IDENTITY();

DECLARE @accountId  INT,@payment DECIMAL(26,6),@amountWholeSale DECIMAL(26,6),@goodId INT,@barcodeId INT,@series NVARCHAR(200),@dateOfProduction DATETIME,@lastExpirationDate DATETIME
,@quantity DECIMAL(26,6),@unitId INT,@priceBuy DECIMAL(26,6),@priceRetail DECIMAL(26,6),@priceWholeSale DECIMAL(26,6),@totalPayment DECIMAL(26,6)=0
DECLARE crs CURSOR FOR 
(SELECT AccountId,ISNULL(Payment,0),[AmountWholeSale],[GoodId],[BarCodeId],[Series],[DateOfProduction],[LastExpirationDate],
[Quantity],[UnitId],[PriceBuy],[PriceRetail],[PriceWholeSale]
FROM @detail)
...
WHILE @@FETCH_STATUS=0
BEGIN
SET @totalPayment=@totalPayment+@amountWholeSale

 
INSERT INTO [Accounting].[DocumentDetail]([DocumentId],[PaymentId],[AccountId],[GoodId],[BarCodeId],[Series],[DateOfProduction],[LastExpirationDate],[Quantity],[UnitId],[PriceBuy],[PriceRetail],[PriceWholeSale],[AmountBuy], [AmountRetail]
,[AmountWholeSale],[Payment],[Deficit]
,[CreateId],[CreateDate])
VALUES(@id,@paymentid,@accountId, @goodId,@barcodeId,@series,@dateOfProduction,@lastExpirationDate,@quantity,@unitId,@priceBuy,@priceRetail,@priceWholeSale,@quantity*@priceBuy,@quantity*@priceRetail,@quantity*@priceWholeSale
,@payment,@amountWholeSale 
,@userId,GETDATE())
...
from [Accounting].[VwDocument] d
 ...
 left join [Accounting].[VwContract] c on c.id=d.ContractId
JOIN dbo.WarehouseMenuCollection wmc ON wmc.WareHouseId = d.WareHouseId and wmc.UserId=@userId AND wmc.Allow=1 AND wmc.MenuId=@menuId
 where c.ContractTypeId=2--aliw ucun
  and d.DeleteId is null and AccountId=5
  --and (@isFiltered=0 OR (@isFiltered=1 and isnull([DocTypeId],0) <>1))
   --AND (@docCertId IS NULL OR [DocCertId]=@docCertId)
 group by 
     d.[Id]   
...
--WHILE @@FETCH_STATUS=0
--BEGIN
--SET @totalAmount=@totalAmount+@amountWholeSale
...
--INSERT INTO Accounting.Stock (GoodId,CreatId,CreateDate,WarehouseId,DocumentId,Quantity,PriceBuy,PrcieRetail,PriceWholeSale) VALUES (@goodId1,@userId,GETDATE(),@wareHouseId,@id,@quantity1,@priceBuy1,@priceRetail1,@priceWholeSale1)
--FETCH NEXT FROM crs INTO @accountId ,@payment1 ,@amountWholeSale ,@goodId1,@barcodeId1,@series1 ,@dateOfProduction1 ,@lastExpirationDate1,@quantity1,@unitId1,@priceBuy1,@priceRetail1,@priceWholeSale1
--end
--CLOSE crs
--deallocate crs
--update [Accounting].[Document]
