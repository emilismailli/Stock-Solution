...
GO
/*
The column [Accounting].[Expense].[Code] on table [Accounting].[Expense] must be added, but the column has no default value and does not allow NULL values. If the table contains data, the ALTER script will not work. To avoid this issue you must either: add a default value to the column, mark it as allowing NULL values, or enable the generation of smart-defaults as a deployment option.
*/

IF EXISTS (select top 1 1 from [Accounting].[Expense])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT

GO
PRINT N'Dropping unnamed constraint on [Accounting].[Expense]...';


GO
ALTER TABLE [Accounting].[Expense] DROP CONSTRAINT [DF__Expense__CreateD__5BD88551];


GO
PRINT N'Altering [Accounting].[Cash]...';
...
GO
PRINT N'Starting rebuilding table [Accounting].[Expense]...';
...
CREATE TABLE [Accounting].[tmp_ms_xx_Expense] (
    ...
    [IsActive]    BIT            NULL,
    CONSTRAINT [tmp_ms_xx_constraint_UIExpenseCode1] UNIQUE NONCLUSTERED ([Code] ASC),
    CONSTRAINT [tmp_ms_xx_constraint_PKRefExpense1] PRIMARY KEY CLUSTERED ([Id] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [Accounting].[Expense])
    BEGIN
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Expense] ON;
        INSERT INTO [Accounting].[tmp_ms_xx_Expense] ([Id], [ParentId], [ExpenseName], [Description], [CreateId], [CreateDate])
        SELECT   [Id],
                 [ParentId],
                 [ExpenseName],
                 [Description],
        ...
        FROM     [Accounting].[Expense]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [Accounting].[tmp_ms_xx_Expense] OFF;
    END

DROP TABLE [Accounting].[Expense];

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_Expense]', N'Expense';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_UIExpenseCode1]', N'UIExpenseCode', N'OBJECT';

EXECUTE sp_rename N'[Accounting].[tmp_ms_xx_constraint_PKRefExpense1]', N'PKRefExpense', N'OBJECT';
...
GO
PRINT N'Altering [Accounting].[VwExpense]...';


GO
ALTER VIEW [Accounting].[VwExpense]
	AS SELECT 
