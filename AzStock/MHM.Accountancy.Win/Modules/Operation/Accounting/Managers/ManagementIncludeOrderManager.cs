...
{
    public partial class ManagementIncludeOrderManager : MHMManager, IManager
    {
        AccountingDataSet temp;
        ManagementIncludeOrderManager(IContainer container, int managerId)
        ...
        public void Loading()
        {

            {
                ...
                vwPaymentTableAdapter.Fill(dataSet.VwPayment);
        ...
        protected override void btnAdd_ItemClick(object sender, ItemClickEventArgs e)
        {
            this.MHMContainer.InvokeAction(delegate
             {
                 using (var form = new ManagementIncludeOrderForm(this, null, OperationType.Add, true))
        ...
        protected override void btnEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            this.MHMContainer.InvokeAction(delegate
            {
                using (var frm = new ManagementIncludeOrderForm(this, spManagementPaymentDebtBindingSource, OperationType.Edit ,true))
                {
        ...
        {
            var row = spManagementPaymentDebtBindingSource.GetBindingRow<AccountingDataSet.spManagementPaymentDebtRow>();
            if (row != null && MHMMessage.ShowQuestionMessageBox("Silmək istədiyinizdən əminsinizmi?") == System.Windows.Forms.DialogResult.OK)
            {
                ...
                .Add("@id", row.Id);
                MHMContainer.CurrentUser.ExecuteProcedure(dataSet.spManagementPaymentDebt, "[Accounting].[spManagementPaymentDebt]", Parameters, true, System.Data.CommandType.StoredProcedure);
                Loading();
            }
