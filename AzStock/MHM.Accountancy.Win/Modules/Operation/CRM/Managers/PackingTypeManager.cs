...
{
    public partial class PackingTypeManager : MHMManager,IManager
    {
         PackingTypeManager(IContainer container, int managerId)
        ...
        {
            using (var frm = new PackingTypeForm(this, null, OperationType.Add, true))
            {
        ...
        {
            var current = vwPackingTypeBindingSource.GetBindingRow<CRMDataSet.VwPackingTypeRow>();

            if (current == null) return;//gridden secim edilmeyib demek

            using (var frm = new PackingTypeForm(this, vwPackingTypeBindingSource, OperationType.Edit))
            {
                if (frm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    MHMContainer.PrintSuccess(CommonLang.Success);
        ...
        protected override void btnDelete_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            {
                vwPackingTypeTableAdapter.DeleteQuery(current.Id);
                MHMContainer.PrintSuccess(CommonLang.Success);
                Loading();
            }
        }
        ...
        protected override void btnShow_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            using (var frm = new PackingTypeForm(this, vwPackingTypeBindingSource, OperationType.None))
            {
                if (frm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    MHMContainer.PrintSuccess(CommonLang.Success);
        ...
        private void gridViewPackingType_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
